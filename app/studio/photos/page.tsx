...
export default async function StudioPhotos(){
  ...
  return <main className="min-h-screen bg-[#f3f2ed] text-[#151515]">
    ...
    <section className="mx-auto max-w-6xl px-5 py-12 lg:px-10"><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-black/40">Catalogue studio</p><h1 className="mt-3 font-serif text-5xl">Mes photographies</h1></div><p className="text-sm text-black/40">{rows.length} image{rows.length!==1?"s":""}</p></div>
      ...
      <div className="mt-10 overflow-hidden rounded-2xl border border-black/8 bg-white"><div className="grid grid-cols-[1fr_140px_120px] gap-4 border-b border-black/8 px-5 py-4 text-xs font-bold uppercase tracking-wider text-black/35"><span>Photographie</span><span>Dimensions</span><span>État</span></div>{rows.map((image)=>{const status=labels[image.status]??{text:image.status,style:"bg-black/5"};return <a href={`/studio/photos/${image.id}`} key={image.id} className="grid grid-cols-[1fr_140px_120px] items-center gap-4 border-b border-black/[.06] px-5 py-5 transition hover:bg-black/[.025] last:border-0"><div className="flex items-center gap-4"><span className="grid h-14 w-14 place-items-center rounded-xl bg-black/[.04]"><ImageIcon size={20} className="text-black/30"/></span><div><p className="font-semibold">{image.title}</p><p className="mt-1 text-xs text-black/40">Original privé · {image.createdAt}</p></div></div><span className="text-sm text-black/55">{image.width} × {image.height}</span><span className={`w-fit rounded-full px-3 py-1.5 text-xs font-semibold ${status.style}`}>{image.status==="PROCESSING"&&<Clock3 className="mr-1 inline" size={13}/>} {image.status==="READY"&&<CheckCircle2 className="mr-1 inline" size={13}/>} {status.text}</span></a>})}</div>}
      <div className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><strong>Pipeline actuel :</strong> les originaux sont privés et les traitements sont enregistrés dans une file. La création automatisée des aperçus et filigranes reste désactivée tant que le moteur d’image n’est pas déployé.</div>
    </section>
  </main>;
}

function State({icon:Icon,title,text,action=false}:{icon:typeof AlertCircle;title:string;text:string;action?:boolean}){return <div className="mt-10 grid min-h-80 place-items-center rounded-3xl border border-dashed border-black/15 bg-white/50 p-8 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-black/[.06]"><Icon/></span><h2 className="mt-5 font-serif text-3xl">{title}</h2><p className="mx-auto mt-2 max-w-md text-sm text-black/45">{text}</p>{action&&<a href="/studio/photos/nouvelle" className="mt-6 inline-flex items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-semibold text-white"><Plus size={16}/>Importer une photographie</a>}</div></div>}
